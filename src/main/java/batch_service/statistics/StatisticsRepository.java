package batch_service.statistics;
...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity, Integer> {
    //batch_service/statistics
    @Query(value = "SELECT new batch_service.statistics.AggregatedStatistics(s.statisticsAt, SUM(s.allCount), SUM(s.attendedCount), SUM(s.cancelledCount)) " +
